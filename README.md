...
Restricted Boltzmann machines (RBM)s are a generative model that can be trained to represent a dataset as a joint probability distribution over its features. This means that stastical samples drawn from the model will look like the types of things found in the dataset. The neural network essentially learns how to "make up" its own examples of the data.
